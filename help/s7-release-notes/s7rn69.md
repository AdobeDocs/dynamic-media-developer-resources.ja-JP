...
* **SSO（シングルサインオン）機能** - Adobe Experience CloudからScene7をシームレスに起動し、Adobe IDを使用してScene7にログインできるようになりました。
   ...
   SSO を設定してScene7で使用できるようにする場合は、Adobeテクニカルサポートにお問い合わせください。 次の情報を用意して、Experience CloudとScene7のアカウントを相互にリンクできるようにします。Adobe Experience Cloud組織名Scene7の会社名およびお客様の地域
...
* 修正 — IE9:s7sdk.event:PageMouseEvent:「 」をクリックし、「イベント」トリガークラッシュブラウザーページをダブルクリックします。
* 追加済み `TableOfContents` コンポーネント。

FlyoutzoomView

* 修正 — Firefox:フライアウトでタイルが読み込まれるまでの間、壊れた画像アイコンが短時間表示されました。
...
* 修正 — 画像セット：assetID に追加された IS 修飾子は無視されます。
...
ステージングサーバーにアクセスするようにコンピューターを設定したら、Web サイトを調べてアップグレードをテストできます。 標準提供のHTML5 ビューアを使用するお客様にとって、ベストプラクティスは、Adobeの標準ステージングサーバーと s7is1-preview-staging.scene7.com に対してテストすることです。
...
* HTML5 MixedMedia ビューア：アセットタイプの指定時に、ズームボタンの状態がリセットされない。
* HTML5 MixedMedia ビューア：最初のアセットが 2dspinset の場合、ビューアがレスポンシブモードで折りたたまれました。
...
* コード内にインスタンス名が明示的に含まれるパラメーターは上書きする必要があります。URL 内のインスタンス名も上書きする必要があります。 例： `zoomView.iconfeffect=0`.
