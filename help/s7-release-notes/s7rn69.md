...
description: 「Adobe Experience CloudのAdobe Experience Manager ソリューションの一部であるAdobe Scene7 Publishing System 6.9 の最新機能、機能強化および修正点について説明する最新のリリースノートです」
...
Adobe Experience CloudのAdobe Experience Manager ソリューションに含まれるAdobe Scene7 Publishing System 6.9 の最新機能、機能強化および修正点について説明する最新のリリースノートです。
...
* [Scene7 Publishing System 6.9 リリースハイライト ](s7rn69.md#section-2ed72b7902be4509876d81f25b5632ce)
...
* **SSO （シングルサインオン）機能** - Adobe Experience CloudからScene7をシームレスに起動し、Adobe IDを使用してScene7にログインできるようになりました。
  ...
  Scene7で使用できる SSO 設定を希望する場合は、Adobeのテクニカルサポートにお問い合わせください。 Experience CloudアカウントとScene7 アカウントをリンクできるように、Adobe Experience Cloudの組織名、Scene7の会社名、地域などの情報を用意します。

## 画像 {#section-98a61ea9a09940639f357f36969d6e55}

* **レスポンシブサポートの強化** - ブレークポイントごとに異なる画像プリセットまたは修飾子を定義することで、レスポンシブな画像エクスペリエンスの品質をさらに最適化します。 [ レスポンシブ静的画像ライブラリの使用 ](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/image-serving-api/image-serving-api/responsive-static-image-library/c-about-responsive-static-image-library.html) を参照してください。
...
* ビデオキャプションが、標準の `Universal_HTML5_MixedMedia_dark` ビューアと `Universal_HTML5_MixedMedia_light` ビューアで使用できるようになりました。 詳しくは、[ビデオへのキャプションの追加](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/video/adding-captions-video.html#video)を参照してください。
...
* Render Server リクエストのセキュアテスト – コア画像サービングアセットおよびビデオアセットのセキュアテストが拡張されて、ビネットも含まれるようになりました。 [ アセットを公開する前のテスト ](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish) を参照してください。
* CDN キャッシュの無効化機能の監視。 CDN 無効化テンプレートを選択し（[ 一般設定 ](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/setup/application-setup.html#general-settings) を参照）、**[!UICONTROL CDN を無効にする]** リクエストを行うと、キャッシュの消去にかかる時間の見積もりでインジケーターがポップアップ表示されるようになりました。

## ビューア {#section-fc0577722d0b431dae3506c8d4e5366f}

**Scene7 HTML5 Viewer SDK 2.8.1 の新機能、機能強化およびバグ修正**
...
ステージングサーバーにアクセスするようにコンピューターを設定したら、Web サイトを確認してアップグレードをテストできます。 標準提供のHTML5 ビューアを使用しているお客様の場合、ベストプラクティスは、Adobeの標準ステージングサーバーおよびs7is1-preview-staging.scene7.comに対してテストすることです。
...
* HTML 5 eCatalog ビューアで目次がサポートされるようになりました。
* HTML 5 eCatalog に、画像マップのオーバーレイとアイコンのサポートを追加しました。
* HTML5 eCatalog ビューアへのページターンアニメーションのサポートが追加されました。
* HTML 5 eCatalog でネイティブページのスクロールがサポートされるようになりました。
* `highlightmode` 修飾子を使用して幅の狭い画像をHTML5 Flyout ビューアに表示する機能を追加しました
* HTML5 の Flyout ビューアへの修飾子を使用して画像のアップスケーリング `zoomfactor` 制御するサポートが追加されました。
* HTML5 Flyout ビューアで生成されるトラッキングイベントをサポートするようになりました。
* HTML5 MixedMedia ビューアでビデオ（最初のビデオのみ）のキャプションがサポートされるようになりました。
* HTML5 ビデオおよび MixedMedia ビューアの初期ビットレートのデフォルトを 1400 に増やしました。
* HTML5 ビデオビューアでビデオ再生をループできるようになりました。
...
* ポスター画像に「最大サイズ」エラーが発生する場合があります。 そのため、画像サービスPublishの制限設定を増やす必要があります。
...
* Scene7パブリッシングシステムは、規則が書き換えられても、グループを記憶し続けました。
* Scene7 パブリッシュシステムが、繰り返し用の特定の正規表現コードを正しく表示しませんでした。
* プリセットを選択していない場合、アップロード中にすべてのバッチセットプリセットが適用されます。

## サポートされなくなった機能 {#section-b061ad3d170a45ee8e694f997fd5d8ef}

* Scene7 Creative Suite拡張機能は、Creative CloudアプリケーションでFlashパネルがサポートされなくなったため、このリリースでは非推奨（廃止予定）となりました。 コンテンツ取得ワークフローのさらなる開発では、Adobe Experience Manager Dynamic MediaとCreative Cloudに重点を置いています。 Scene7の場合、Scene7 パブリッシュシステムのアップロード機能およびエクスポート機能は引き続き使用でき、代わりに使用できます。
* UGC ビデオは、採用が少ないため、このリリースでは非推奨（廃止予定）となりました。 別の SPS アカウントを設定すると、UGC ビデオの代替ソリューションとして使用できます。 別のアカウントを設定するには、Adobeのテクニカルサポートにお問い合わせください。
* テキストハイフネーションは、採用が少ないため、このリリースでは非推奨です。 ハイフネーションの辞書では、最適なテキストフローをサポートするために、単語を分割したりハイフネーションを行ったりする方法を定義しました。
* メタデータサーバーと類似性検索は、導入が少なかったため、2015 年に非推奨（廃止予定）になりました。 正式な廃止予定日が発表された際には、影響を受けるお客様にはAdobeのテクニカルサポートから通知が届きました。
* 注意：2014 年 1 月 31 日（PT）をもって、Scene7は DHTML ビューアプラットフォームのサポートを正式に終了しました。
