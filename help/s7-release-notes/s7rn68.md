...
* 高度なHTML5 ビューアのカスタマイズ — 埋め込む Web ページとカスタムビューアのインタラクションを作成します。
...
* HTML5 eCatalog ビューアのマウスドラッグ&amp;ドロップが、画像マップを含むページで Internet Explorer で機能しない。
...
* 「Space After」は機能しません。使用する必要があります `sa`、現在は `margb`.
...
* ユーザー定義フィールドの日付/時間の値を削除しても、Scene7 Publishing System は IPS(Image Publishing System) にリクエストを送信しません。
...
* 動的 (Flash) バナーは、このリリースで非推奨（廃止予定）となりました。 パーソナライズされた Web サイトバナーの作成、管理、配信に関する他のテクノロジーが登場したので、この機能の採用は低くなっています。 Scene7 Dynamic(Flash) バナー機能は、サーバー側で引き続き機能しますが、サポートされなくなりました。
* リマインダー：メタデータサーバーと関連する API の使用は、以前のScene7リリースで正式にサポートを終了しました。
...
* 最小要件をiOS6 に増やしました。
* 固定：全画面モードで、Safari のツールチップが画面外に表示される。
...
* 固定：全画面から戻ってアセットを指定し直した後に、コンテナのサイズが正しくない問題を修正しました。
* 固定：CSS 変換でコンテナがシフトされる場合、コンテナの座標が正しくありません。

**EmailShare**

* 固定：削除ボタンで発生するツールチップアーティファクト。
* 固定：スクロールバーのツールヒントがユーザーインターフェイスの後ろに表示され、切り抜かれます。
* 固定：「キャンセル」ツールヒントが「メールを送信」ボタンの後に表示される

**EmbedShare**

* 固定：埋め込みコードテキストが折り返されず、Internet Explorer のスクロールバー上に表示される。

**FlyoutZoomView**

* 固定：Internet Explorer 8 では、Flyout イベントはディスパッチされません。
...
* 固定： `selectSwatch` メソッドが `initViewer()` 関数に置き換えます。

**SocialShare**

* 共有ボタンのレイアウトの位置を制御する軸受修飾子のサポートを追加しました。
* ソーシャル機能を操作する際に、一時停止/再生を制御するイベントを追加しました。
* 固定：ソーシャル共有アイコンが `controlbar` ズーム時
* 固定：Galaxy SII で初めてフェードインしたときに、ソーシャルシェアアイコンが表示されない。
* 固定：iPad3 では、ソーシャル共有モーダルダイアログがフルスクリーンモードで表示されません。
* 固定：Safari 5.1 を使用しているMacで、ソーシャル共有モーダルダイアログがフルスクリーンモードで表示されない。
* 固定：スクロール後にソーシャル共有モーダルダイアログが画面外に表示される。

**ThumbnailGridView**

* 固定：ThumbnailGridView は送信されません `AssetEvent.ASSET_CHANGED`.
...
* 固定：HTTPS プロトコルで、Flash再生を使用して IE8 でセキュリティ警告を生成します。
* 固定：Safari 5 のフルスクリーンモードで、ビデオの再生を使用すると、ビデオのサイズが正しくFlashされません。
* 固定：iPhoneでビデオを再生した後は、メディアセット内のスウォッチを選択できません。

**ZoomView**

* 固定：Android™で画像をズームした後、またはデバイスを回転させた後に表示されるアーティファクト。

## S7OnDemand 5.0.1 の新機能とバグ修正 {#section-ba3a595134144711bf27dd8dca249e5d}

* 固定：HTML5 および AS3 ビューアの HTTPS のサポート。
