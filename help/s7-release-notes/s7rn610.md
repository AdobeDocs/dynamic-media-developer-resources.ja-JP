...
description: 「Adobe Experience CloudのAdobe Experience Manager ソリューションの一部であるAdobe Scene7 Publishing System 6.10 の新機能、機能強化および修正点について説明する最新のリリースノートです」
...
Adobe Experience CloudのAdobe Experience Manager ソリューションに含まれるAdobe Scene7 Publishing System 6.10 パートの新機能、機能強化および修正点について説明する最新のリリースノートです。
...
* **標準提供のHTML5 スピンセットビューアに新しい設定が追加されました** - **[!UICONTROL 設定]**/**[!UICONTROL アプリケーション設定]**/**[!UICONTROL ビューア]**/**[!UICONTROL ビューアプリセット]**/`Universal_HTML5_SpinSet_dark` または `Universal_HTML5_SpinSet_light` で、各ビューアの各 [!DNL Configure Viewer] インページに次の設定が追加されました。
...
* **標準提供のHTML5 eCatalog ビューアに新しい設定が追加されました** - **[!UICONTROL 設定]**/**[!UICONTROL アプリケーション設定]**/**[!UICONTROL ビューアプリセット]**/`Universal_HTML5_eCatalog` または `Universal_HTML5_eCatalog_Adv` で、各ビューアの各 [!DNL Configure Viewer] ページに次の設定が追加されました。
...
* IPS Classic Publishの「検証」内の「カタログサーバーの再同期」オプションを削除しました。
...
ビューアのアップグレードは後方互換性があるため、既存の web コードを変更する必要はありません。 ただし、新しいビューアをAdobeのステージング環境でテストすることをお勧めします。 次の web サイトでは、Adobeのステージングサーバーにアクセスするためのシステムの設定方法を説明しています。

[https://helpx.adobe.com/experience-manager/scene7/kb/base/upgrade-management/testing-scene7-viewers-upgrade.html](https://helpx.adobe.com/experience-manager/scene7/kb/base/upgrade-management/testing-scene7-viewers-upgrade.html)

ステージングサーバーにアクセスするようにコンピューターを設定したら、Web サイトを確認してアップグレードをテストできます。 標準提供のHTML5 ビューアを使用する場合は、Adobeの標準のステージングサーバーおよびs7is5-preview-staging.scene7.comに対してテストすることをお勧めします。
...
* デスクトップブラウザーのHTML5 スピン、BasicZoom、Zoom、スピン、MixedMedia ビューアに対する CSS サイズマーカーが削除されました。
...
**Scene7 HTML5 Viewer SDK 2.9.2 の新機能、機能強化およびバグ修正**

一般

* s7sdk.browser.device.version が第 5 のAndroid™ を誤って検出していた問題を修正しました。
* SDK コンポーネントの表示/非表示スタイル設定の使用を削除しました。
...
* Publish設定に、Scene7 Shared Assets company のカラープロファイルが表示されませんでした。
* FTP からアップロード画面：FTP フォルダーは並べ替えられませんでしたが、出力されたフォルダーリストは並べ替えられました。
* メディアポータル設定の編集グループの「アセットアクセス権限」タブの説明を変更しました。
* Secure Test Image Serving Publish設定を使用している間に、無効な形式の `clientAddressFilter` が原因でカタログサーバーから例外が発生していました。
* URL に複数の連続したスペースが存在する場合、Chrome ブラウザーでPDF用に URL をコピーすると失敗します。
...
* Scene7 パブリッシュシステムの web から印刷へのダウンロードエクスペリエンスに対して行われた改良。
...
* 画像サービング Publishタイプのオプションを追加しました。
...
* **Flashビューアの提供終了のお知らせ***2017 年 1 月 31 日（PT）をもって、Adobe Scene7（現在のDynamic Media Classic）はFlashビューアプラットフォームのサポートを正式に終了しました。*
...
* UGC ビデオは、採用が少ないため、このリリースでは非推奨（廃止予定）となりました。 別のDynamic Media Classic アカウントを設定することは、UGC ビデオの代替ソリューションとして機能します。 別のアカウントを設定するには、Adobeのテクニカルサポートにお問い合わせください。
* テキストハイフネーションは、採用が少ないため、このリリースでは非推奨です。 ハイフネーションの辞書では、最適なテキストフローをサポートするために、単語を分割したりハイフネーションを行ったりする方法を定義しました。
* メタデータサーバーと類似性検索は、どちらも導入が少なかったため、2015 年に非推奨（廃止予定）になりました。 正式な廃止予定日が発表された際には、影響を受けるお客様にはAdobeのテクニカルサポートから通知が届きました。
