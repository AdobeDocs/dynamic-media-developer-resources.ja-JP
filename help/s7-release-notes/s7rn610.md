...
[!DNL Adobe Scene7 Publishing System] の [!DNL Adobe Experience Manager] ソリューションの 6.10 パートの新機能、機能強化および修正点 [!DNL Adobe Experience Cloud] ついて説明する最新のリリースノートです。
...
<table id="table_A48D69C0F26D43B58C4C890FBF908F8A"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> <p> <span class="uicontrol"> StartsWith </span> </p> </td>
   <td colname="col2"> <p>文字列の先頭から検索し、「次を含む」より高速 <span class="uicontrol"> 検索結果を返し </span> す。 </p> <p>新規ユーザーは、個人用セットアップ <span class="uicontrol"> ージでこの既定値 </span> 上書きできます。 </p> </td>
  </tr>
 </tbody>
</table>
...
<table id="table_E78C924E34894CD7BE080C0D71500784"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>自動スピンアニメーションを有効または無効にします。 デフォルトは <span class="uicontrol"> Off </span> です。 「<span class="uicontrol"> On </span>」に設定した場合は、スピンの持続時間（1 回のフルスピンの秒数）を指定できます。デフォルトは <span class="codeph"> 5 </span> です。 スピンの方向を右または左に設定します。デフォルトは右 <span class="uicontrol"> で </span>。 </p> </td>
  </tr>
  <tr> 
   <td colname="col1"> <p> <span class="uicontrol"> Rotation count </span> </p> </td> 
   <td colname="col2"> <p>Autospin が停止する前の完全な回転の数である <span class="uicontrol"> Rotation count </span> を設定できます。デフォルトは 1<span class="codeph"> 間 </span> す。 </p> <p>回転の回数を <span class="codeph">-1 に設定すると、自動スピン </span> 無限になります。 </p> </td>
  </tr>
  <tr> 
...
<table id="table_65BA9F98D1D041109A44B1C130EF4EFA"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>このタブには、<span class="uicontrol"> Expiration time </span> およびお気に入りのサムネールサイズ <span class="uicontrol"> （） </span> 関する設定が含まれています。 </p> <p> 
...
<table id="table_023203E3CC484F59A350C3C333EF817F"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>使用するズーム アニメーションのタイプを設定できます。 左側のパネルで「ビューア機能」 <span class="uicontrol"> プション </span> クリックして、ドロップダウンリストから次の <span class="uicontrol"> ズームモード </span> オプションにアクセスします。 </p> <p> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 左側のパネルで、フライアウト/インラインズーム <span class="uicontrol"> ージ </span> クリックして、次の使用可能なオプションにアクセスします。 </p> <p> 
     <ul id="ul_081A473EFAAC47DD8CF5260492A53C67"> 
      <li id="li_A672C11207524F8AB7957B6848F4FFF1"> <span class="uicontrol"> プライマリのズーム率 </span> （デフォルト：<span class="codeph"> 3 </span>）と <span class="uicontrol">セカンダリのズーム率 </span> （オプション：<span class="codeph"> -1 </span>） – フライアウト/インラインビューの、リセットビューを基準にした画像倍率を指定します。 整数または 1.0<span class="codeph"> 以上の浮動小数点値 </span> 指定します。 <p>オプションの 2 次ズーム倍率を指定できます。 ハイライトがアクティブな状態でメイン ビューを選択すると、2 次ズームにアクセスできます。 2 回目に選択すると、プライマリズーム率に戻ります。 値が <span class="codeph">-1 の場合、2 次ズーム倍率 </span> 無効になります。 </p> </li> 
...
* API リク `<metadataArray>` ストの応答で [!DNL .vtt] ファイルの `getAssets` が空でした。
...
* CSS で IMG に max-width:100% が定義されている場合、IE で ZoomView – 壊れたズームとパンが行われます。
* 追加のステータスイベントを追加しました。
...
* タグタイプのメタデータフィールドを作成および更新するための設定ユーザーインターフェイスを実装しました。 **[!UICONTROL パラメーターでの]** メタデータフィールドタイプ `fieldType` のサポートには、`SingleFixedTag` と `BooleanTag` のみが含まれます。
...
* **[!UICONTROL タイプのユーザー定義フィールド（]** 設定 **/** メタデータ **/** ユーザー定義フィールド `Boolean`）に値 `true` を追加して保存すると、エラーが発生しました。
