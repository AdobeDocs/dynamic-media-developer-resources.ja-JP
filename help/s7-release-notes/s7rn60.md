...
description: Adobe Marketing CloudのAdobe Experience Manager ソリューションの一部である Adobe Scene7 Publishing System 6.0 の機能、機能強化および修正点について説明するリリースノートです。
...
Adobe Marketing Cloudに含まれる Adobe Scene7 Publishing System 6.0 の機能、機能強化および修正点について説明するリリースノートです。
...
* このリリースに続いて、新しいインスタント公開機能が徐々に展開されています。 この機能は、現在の「公開用にマーク」機能に代わるものです。 一部のユーザーには、他のユーザーよりも先に新しい機能が表示される場合があります。 SPS のドキュメントと UI 要素は、ロールアウト期間が完了に近づいたときに更新される予定です。 ロールアウト開始時に、詳細を含むメールが届く予定にできます。
...
* HTML5 SDKの広範な新しいビューアコンポーネント。 （詳しくは、インストーラーに含まれているHTML 5 SDKの Readme を参照してください）。
* 拡張 Flash SDK。 （詳しくは、インストーラーに含まれる Flash SDKの Readme を参照してください）。
* 新しい Flash AS3 ビューアには次のものが含まれます。
...
* 新しいHTML5 ビューアには次のものが含まれます。

   * ズーム
   * 画像セット
   * スウォッチセット

* テキストやグラフィックのローカリゼーションなど、ビューアのグローバリゼーションのサポートを追加しました。 SDK インストーラーに付属している Flash またはHTML5 ビューアのSDK ユーザーガイドを参照してください。
* 埋め込みビデオビューアおよびスピンビューアを使用した eCatalog へのインタラクティブ機能の追加。
* Adobe Developer Connectionから Flash およびHTML5 ビューア SDK へのアクセスが簡素化されました。<!-- [More information](http://help.adobe.com/en_US/scene7/using/WSd4272150f67705c11b002eec12fcba4dee6-8000.html). -->
...
* Adobe CQ5 との統合：アセットの同期、CQ5 に埋め込まれた Scene7 ビューア、Scene7 ネットワークを介した CQ5 ページへのリッチメディアコンテンツの動的配信。
* Scene7/Creative Suite – 今後のAdobe Creative Suite 6 のリリースと互換性があります。
...
* テンプレートセット - Scene7 基本テンプレートの様々なバリエーションを Scene7 公開システム内から直接簡単に管理でき、ページコードで URL を変更する必要がなくなります。<!-- [More information](http://help.adobe.com/en_US/scene7/using/WSd968ca97bf00cf72-5eede3a113268dc80f5-8000.html).  -->
* 基本テンプレートエディターの「開発者モード」 - Scene7 パブリッシュシステムで複雑なテンプレート URL を直接編集し、組み込みのプレビューを使用して結果を即座にテストします。
* ダイナミックバナーの Flash ソースコンテンツ（FLA）を、高度なフレーム管理を含むアニメーションのGIF ムービーに動的にコンパイルします。<!-- [More information](http://help.adobe.com/en_US/scene7/using/WSd968ca97bf00cf72-5eedd3a113268dc80f4-8000.html).  -->
