---
description: 'カタログレコードのID: マテリアル レコードがレンダーサーバーによって参照されるキー値。 通常は、SKU番号などの短く一意のマテリアル識別子を使用します。'
solution: Experience Manager
title: ID
feature: Dynamic Media Classic,SDK/API
role: Developer,User
exl-id: 2525a9c9-8acb-45a4-b6d1-80d08f63f9f8
TQID: 'https://experienceleague.adobe.com/DNvokf6XnX6yZfhI5Cf1sE1f8r-TFGMgFZIW-1ZpxGY'
product_v2: id: fd1f54a9-f50c-467d-8956-cebbaf4f3eb8
feature_v2: id: a01bfd36-4ab8-4bf8-9dc0-5b45b890552e
role_v2: id: b69b2659-1057-424e-8fc5-ed9e016dc554id: ff6a42d2-313e-452e-93a6-792e4fad9ff8
source-git-commit: 49c3ac586f6fb17608838f8dcf2c637822314fc7
workflow-type: tm+mt
source-wordcount: 81
ht-degree: 8%

---

# ID{#id}

カタログレコードのID: マテリアル レコードがレンダーサーバーによって参照されるキー値。 通常は、SKU番号などの短く一意のマテリアル識別子を使用します。

## プロパティ {#section-9b20bf2545314620808b4210bce7afc9}

テキスト文字列。 必須。 各`catalog::Id`値は、このマテリアル カタログ内で一意である必要があり、「,」文字を含めることはできません。

## 初期設定 {#section-6a517046c43a40a7ba0af0212b5f31d2}

なし

## 関連項目 {#section-4a44c8b070674ce3a7f2c13a3003a14a}

[属性：:RootId](../../../../../ir-api/material-cat/image-rendering-api-ref/c-ir-material-catalog/c-ir-attributes-reference/r-ir-rootid.md#reference-54b42b7125824be593378c1accb70d5a)、[src=](../../../../../ir-api/http-protocol/image-rendering-api-ref/c-ir-http-protocol-ref/c-ir-http-protocol-command-reference/r-ir-src.md#reference-62c98abad22149d68d405ed6aaff8272)

