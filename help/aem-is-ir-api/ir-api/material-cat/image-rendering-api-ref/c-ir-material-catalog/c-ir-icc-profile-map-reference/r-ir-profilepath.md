---
description: プロファイルファイルファイルパス： ICC カラープロファイルファイルのパスと名前（ファイルサフィックスを含む）。
solution: Experience Manager
title: ProfilePath
feature: Dynamic Media Classic,SDK/API
role: Developer,User
exl-id: f3ab11a6-d14d-426b-aaa8-af14f062532c
TQID: 'https://experienceleague.adobe.com/C3uB6Yc8WhwxatyX03ER3e664mFtrs7lIFH3aP3lZ1w'
product_v2:
  - id: fd1f54a9-f50c-467d-8956-cebbaf4f3eb8
feature_v2:
  - id: a01bfd36-4ab8-4bf8-9dc0-5b45b890552e
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
  - id: ff6a42d2-313e-452e-93a6-792e4fad9ff8
source-git-commit: 49c3ac586f6fb17608838f8dcf2c637822314fc7
workflow-type: tm+mt
source-wordcount: 55
ht-degree: 9%

---

# ProfilePath{#profilepath}

プロファイルファイルファイルパス： ICC カラープロファイルファイルのパスと名前（ファイルサフィックスを含む）。

## プロパティ {#section-5b75d459083f49f3bbd89ce8458559f7}

テキスト文字列。 必須。 有効なImage Server ファイルパスである必要があります。絶対パスまたは`attribute::RootPath`からの相対パスです。

## 初期設定 {#section-67b79d62b3e24c62aff6844cc5e04251}

なし

## 関連項目 {#section-18bdd258f6de456098bd5637064bd1b5}

[属性：:RootPath](../../../../../ir-api/material-cat/image-rendering-api-ref/c-ir-material-catalog/c-ir-attributes-reference/r-ir-rootpath.md#reference-a4d7c96b62e14fcbad1740c702f160f3)

