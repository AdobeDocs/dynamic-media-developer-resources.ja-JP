...
seo-description: 変更のタイムスタンプ。 ビネットが最後に変更された日時を指定します。
seo-title: TimeStamp
...
を設定 `attribute::UseLastModified` した場合、ビネットと要求に関わるすべてのマテリアルの最新の値と `vignette::TimeStamp``catalog::TimeStamp`値が、HTTP応答内で最後に変更されたヘッダーとして返されます。
