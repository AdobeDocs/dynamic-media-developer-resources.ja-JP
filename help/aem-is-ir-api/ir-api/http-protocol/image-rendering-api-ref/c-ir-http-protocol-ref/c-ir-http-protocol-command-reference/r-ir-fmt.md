...
*`pixelType`* を指定しない場合、出力されるカラースペースの変換 `icc=` に影響します。に対応するデフォルトのカラープロファイル *`pixelType`* が適用されます。 カラーマネジメントが無効になっている場合は、純粋な変換が適用されます。 *`pixelType`* を指定した場合は無視され `icc=` 、出力ピクセルのタイプが決まります。
