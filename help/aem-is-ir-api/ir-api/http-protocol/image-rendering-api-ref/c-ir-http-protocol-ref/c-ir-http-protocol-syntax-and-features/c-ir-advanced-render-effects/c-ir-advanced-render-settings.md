---
title: レンダリングの詳細設定
description: ビネットオーサリングツール（Dynamic Media 画像オーサリングパッケージの一部）は、ビネットレンダリングエンジンの低レベル要素を制御するメカニズムを提供します。
...
ビネットオーサリングツール（Dynamic Media 画像オーサリングパッケージの一部）は、ビネットレンダリングエンジンの低レベル要素を制御するメカニズムを提供します。

>[!NOTE]
>
>レンダリング設定は、画像レンダリングと画像オーサリングの詳細機能です。 レンダリング設定の使用に関するトレーニング、コンサルティング、またはその両方について、Adobe テクニカルサポートまたはAdobe コンサルティング担当者にお問い合わせください。
