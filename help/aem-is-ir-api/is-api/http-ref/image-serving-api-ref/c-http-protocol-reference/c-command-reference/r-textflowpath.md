---
title: textFlowPath
description: テキストフロー領域： textPs=で指定したテキストを流す1つ以上の領域を指定します。
...
テキストフロー領域： textPs=で指定したテキストを流す1つ以上の領域を指定します。
...
`textFlowPath=`が存在する場合、RTF マージンコマンド `\margl`、`\margr`、`\margt`および`\margb`は無視されます。 パス定義が指定されていない場合、`textFlowPath=`は無視されます。

## プロパティ {#section-b68dc887c6534ce8982cad740b3aeaa4}

テキストレイヤー属性（`textPs=`のみ）。 他のレイヤーでは無視されます。 `layer=comp`に指定されている場合は、`layer=0`に適用されます。

## 初期設定 {#section-68c4559b9e8242059b82e5a39a455dfc}

レイヤーの長方形と同じです。テキストはレイヤーの長方形全体に塗りつぶされます。
