---
description: 次のサーバー設定を使用して、アクセスをログに記録します。
seo-description: 次のサーバー設定を使用して、アクセスをログに記録します。
seo-title: アクセスログ
...
次のサーバー設定を使用して、アクセスをログに記録します。

構文

## TC::directory — ログファイルフォルダ {#section-5d9e2168d4504bbe9868b7d6051c9d67}

プラットフォームサーバーがログファイルを書き込むフォルダーです。 絶対パスまたは相対パスを指定できます *`install_folder`*。 初期設定は [!DNL  *`install_folder`*/logsです]。

>[!NOTE]
>
>この設定を変更する前に、新しいフォルダを作成する必要があります。 画像サービングがroot以外のユーザーアカウントで実行するようにインストールされている場合は、フォルダーに正しい読み取り/書き込みアクセス権限があることを確認します。
...
アクセスログデータの書き込み先のファイルの名前プレフィックス。 日付とファイルのサフィックス( [!DNL  *`yyyy`*- *`mm`*- *`dd`*-].log)が指定した文字列に追加されます。 アクセスログファイルの名前は、トレースログファイルの名前とは異なる名前にする必要があります。 初期設定は &quot; `access-`&quot;.

## TC::pattern — アクセスログパターン {#section-22775ea85cee444d8a7d7336a3b1feef}

プラットフォームサーバーのアクセスログレコードのデータパターンを指定します。 パターン文字列は、対応する値で置き換えられる変数を指定します。 パターン文字列内の他のすべての文字は、文字どおりログレコードに転送されます。

キャッシュ・ウォームアップ・ユーティリティを使用するには、フィールドの区切り文字としてスペースを使用する必要があります。 プラットフォームサーバーでは、フィールド値内のすべてのスペースと「%」文字が、それぞれと `%20` に置き換 `%25`えられます。
...
<table id="table_7A07AFF34B5040A5B30F5735CFE9428F"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>HTTPヘッダーを除く応答バイト数。ゼロの場合は' '。 </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> %B </span> </p> </td> 
  ...
  <tr> 
   ...
   <td> <p>スレッドID（デバッグ/エラーログエントリを相互参照する場合）。 </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> %G </span> </p> </td> 
   <td> <p>日付と時刻、 <span class="codeph"><span class="varname"> yyyy </span>- <span class="varname"> MM </span>MM <span class="varname"> HH形式 </span> の <span class="varname"></span>HDD <span class="varname"> mm </span>: <span class="varname"> ss </span>. <span class="varname"> SSSオフセ </span> ット </span> </p> <p> ( <span class="varname"> SSS </span> are msec, <span class="varname"> offset </span> is the GMT time offset);時間値は、応答がクライアントに送信されたときに取り込まれます。 </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> %m </span> </p> </td> 
   <td> <p>リクエストメ <span class="codeph"> ソッ </span>ド(GET、 <span class="codeph"> POST </span>など)。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td> <p> <span class="codeph"> %R </span> </p> </td> 
   <td> <p>%rと同じで <span class="codeph"> すが、ロ </span>グ解析の問題を回避するために、URIに限定的なHTTPエンコーディングを適用します。 </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> %s </span> </p> </td> 
   <td> <p>HTTP応答のステータスコード。 </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> %S </span> </p> </td> 
  ...
  <tr> 
   ...
   <td> <p>Platform Server cache managementキーワード： <span class="codeph"> { REUSED|作成済み|更新済み|リモート| REMOTE_CREATED| REMOTE_UPDATED| REMOTE_CACHE|検証済み| IGNORED| UNDEFINED } </span>. </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>etag応 <span class="codeph"> 答ヘ </span> ッダー値（応答データのMD5署名）。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>要求の解析および画像カタログの参照に要した時間。 </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> %{PathBasedAccess}r </span> </p> </td> 
   <td> <p>この要求が、カタログシステム外でパスベースのアクセスを試みたかどうかを示します。 </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> %{PeerServer}r </span> </p> </td> 
   <td> <p>キャッシュエントリを配信したキャッシュクラスタ内のピアサーバーのIPアドレス。 <span class="codeph"> CacheUseが </span> REMOTE_CREATEDでもREMOTE_UPDATEDでもない場合は「 — 」 <span class="codeph"> も </span> 返されま <span class="codeph"></span>す。 </p> </td> 
  </tr> 
  <tr> 
   ...
   <td> <p>エラーカテゴリ： </p> <p> 
     <ul id="ul_BA2A18337D374939AC9BF2424247E40F"> 
      ...
      <li id="li_CCEE27F75BD34195895428188B2C30AA"> <p>1=画像がサーバー上で見つかりません。 </p> </li> 
      ...
      <li id="li_5AFFB0EE80484885BCDACD9DF3EF58F7"> <p>4=一時的なサーバーの過負荷のため要求が拒否されました。 </p> </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> %{ReqType}r </span> </p> </td> 
   <td> <p>req=の大文字の <span class="codeph"> 値です </span>。 </p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>出力ストリームにデータを書き込んだ後、Platform Serverが応答を送信するのにかかる時間。 </p> </td> 
  </tr> 
  <tr> 
   <td> <p> <span class="codeph"> %{Size}r </span> </p> </td> 
   <td> <p>「%B」 <span class="codeph"> と同 </span>様ですが、304（未変更）応答の値が含まれます。 </p> </td> 
  </tr> 
  <tr> 
