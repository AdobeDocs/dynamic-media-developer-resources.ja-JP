...
TQID: 'https://experienceleague.adobe.com/fhluBRLnMS2xNrd2u8A0WBtJV0-zwru78lNfTcXTwW0'
...
>[!NOTE]
>
>このボタンは、`state`と`selected`の両方の属性セレクターをサポートしており、異なるスキンを異なるボタンの状態に適用するために使用できます。 特に、`selected='true'`は、ユーザーが選択またはタップして新しいお気に入りアイコンを追加できる状態に対応しています。 一方、`selected='false'`は、ユーザーがページをズーム、パン、スワップできる通常の操作モードに対応します。
...
例 – 28 x 28 ピクセルの「お気に入りを追加」ボタンを設定し、選択または選択されていない場合に、4つの異なるボタンの状態ごとに異なる画像を表示する。
