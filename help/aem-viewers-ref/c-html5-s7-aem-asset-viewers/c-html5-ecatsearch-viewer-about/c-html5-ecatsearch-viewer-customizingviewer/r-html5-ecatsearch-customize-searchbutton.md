...
TQID: 'https://experienceleague.adobe.com/4isGidoN0mJG46pDeAmgLqLr-8SZihdjUBEJ4HkBKVU'
product_v2:
...
>[!NOTE]
>
>このボタンは、`state`および`selected`属性セレクターをサポートしています。これらのセレクターを使用すると、異なるスキンを異なるボタンの状態に適用できます。
>
>特に、`selected='false'`は最初のスクロールボタンの状態に対応し、`selected='true'`は検索パネルがアクティブな状態に対応します。
...
例 – 28 x 28 ピクセルの検索ボタンを設定し、選択または選択されていない場合に、4つの異なるボタンの状態ごとに異なる画像を表示します。
