...
>[!NOTE]
>
>このボタンでは、属性セレ `state` クターと `selected` 属性セレクターがサポートされます。このセレクターは、ボタンの状態ごとに異なるスキンを適用するのに使用できます。
>
>特に、は初期ス `selected='false'` クロールボタンの状態に対応し、検索パ `selected='true'` ネルがアクティブな状態に対応します。
