...
>[!NOTE]
>
>このボタンは、 `state` および `selected` 属性セレクター。ボタンの状態ごとに異なるスキンを適用するのに使用できます。
>
>特に `selected='false'` は、最初のスクロールボタンの状態に対応し、 `selected='true'` は、検索パネルがアクティブな状態に対応します。
...
例 — 28 x 28 ピクセルで、選択時または未選択時の 4 つのボタンの状態ごとに異なる画像を表示する検索ボタンを設定するには、次のように記述します。
