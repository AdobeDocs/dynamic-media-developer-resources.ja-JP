...
>[!NOTE]
>
>コンボボックスでは、`expanded` および `true` の可能な値を持つ `false` 属性セレクターがサポートされています。 値 `true` は、コンボボックスに事前定義済みの埋め込みサイズのいずれかが表示されるときに使用されるので、使用可能なすべての幅をとる必要があります。 この値 `false`、コンボ ボックスで [ サイズの設定 ] オプションが選択されている場合に使用されます。この値は、幅と高さのカスタム入力フィールドのスペースを確保するために小さくする必要があります。
