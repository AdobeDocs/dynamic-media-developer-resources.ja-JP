...
>[!NOTE]
>
>このボタンでは、 `state`属性セレクターがサポートされます。このセレクターは、ボタンの状態ごとに異なるスキンを適用するのに使用できます。 特に、 `selected='default'`はデフォルトのビデオチャプターマーカーの状態に対応し、 `selected='over'`は、マウスを置くかタッチジェスチャによってビデオチャプターマーカーがアクティブになったときに使用されます。
