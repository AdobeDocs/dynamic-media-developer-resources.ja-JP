---
title: 再生/一時停止ボタン
description: 再生/一時停止ボタンを使用すると、ユーザーがビデオコンテンツをクリックしたときに、ビデオプレーヤーがビデオコンテンツを再生または一時停止します。
...
再生/一時停止ボタンを使用すると、ユーザーがビデオコンテンツをクリックしたときに、ビデオプレーヤーがビデオコンテンツを再生または一時停止します。

<!--<a id="section_061E550C1C1D4DB2BD663A898895B38C"></a>-->

ボタンを含むコントロールバーに対して、CSSでボタンのサイズを変更し、スキンを設定し、位置を調整することができます。

次のCSS クラスセレクターは、ボタンの外観を制御します。

```
.s7videoviewer .s7playpausebutton
```

## 再生/一時停止ボタンのCSS プロパティ {#css-properties-of-the-play-pause-button}
...
>[!NOTE]
>
>このボタンは、`state`、`selected`および`replay`属性セレクターの両方をサポートしており、異なるスキンを異なるボタンの状態に適用するために使用できます。 特に、`selected='true'`は「再生」状態に対応し、`selected='false'`は「一時停止」状態に対応します。
>
...
コントロールバーの上端と左端から6 ピクセル位置に32×32 ピクセルの再生/一時停止ボタンを設定する。 最後に、選択または選択されていない4つの異なるボタンの状態ごとに異なる画像が表示されます。
