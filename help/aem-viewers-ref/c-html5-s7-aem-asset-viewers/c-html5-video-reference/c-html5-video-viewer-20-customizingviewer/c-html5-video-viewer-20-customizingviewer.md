...
カスタムCSSを作成する場合、ビューアはコンテナのDOM要素に`.s7videoviewer`クラスを割り当てることに注意してください。 `style=`コマンドで渡された外部CSSファイルを使用する場合は、CSSルールの子孫セレクターで`.s7videoviewer`クラスを親クラスとして使用します。 Webページにスタイルを埋め込む場合、このセレクターを次のようにコンテナのDOM要素のIDで修飾します。

`#<containerId>.s7videoviewer`

## レスポンシブデザインCSSの構築 {#section-63e8f93ee2f14fd8bba1ce33a6870b80}

CSSでは、異なるデバイスをターゲットにして、ユーザーのデバイスに応じてコンテンツの表示を変えることができます。 このターゲット設定には、ユーザーインターフェイス要素のサイズやアートワークの解像度が異なる場合も含まれますが、これらに限定されません。
...
モバイルビューアに適用する場合は、4つのCSSメディアクエリを使用します。CSS内では以下の順序で定義されます。
...
CSSメディアクエリアプローチを使用する場合は、デバイス検出を行うCSSを次のように整理する必要があります。

* まず、デスクトップ固有のセクションで、デスクトップに固有またはすべての画面に共通のすべてのプロパティを定義します。
* 次に、4つのメディアクエリを上記の定義の順に実行し、対応するデバイスタイプに固有のCSSルールを指定します。
...
* CSS内で指定されている外部アセットへのパスは、ビューアのHTMLページの場所ではなく、CSSの場所を基準に解決されます。 初期設定のCSSを別の場所にコピーする場合は、このルールを考慮してください。 デフォルトのアセットをコピーするか、カスタムCSS内でパスを更新します。
