...
カスタムCSSを作成する場合、ビューアはコンテナのDOM要素にク `.s7videoviewer` ラスを割り当てることに注意してください。 コマンドで渡される外部CSSファイルを使用する場合は、CSSル `style=` ールの子孫セレ `.s7videoviewer` クターでクラスを親クラスとして使用します。 Webページにスタイルを埋め込む場合、このセレクターを次のようにコンテナのDOM要素のIDで修飾します。

`#<containerId>.s7videoviewer`

## レスポンシブデザインCSSの構築 {#section-63e8f93ee2f14fd8bba1ce33a6870b80}

CSSでは、ターゲットデバイスごとにコンテンツの表示を変えることができます。 このターゲット設定には、ユーザーインターフェイス要素のサイズやアートワークの解像度の変化が含まれますが、これに限定されません。
...
以下のサンプルCSSでは、再生/一時停止ボタンのサイズを、マウス入力のシステムでは28 x 28ピクセルに、タッチデバイスでは56 x 56ピクセルに設定します。 また、ビューアのサイズが非常に小さくなると、ボタンは完全に非表示になります。
...
モバイルビューアに適用する場合は、4つのCSSメディアクエリを使用します。このクエリは、CSS内で次に示す順序で定義されます。
...
CSSメディアクエリアプローチを使用する場合は、次のようにデバイス検出を行うCSSを整理する必要があります。
...
* CSS内の外部アセットへのパスは、ビューアのHTMLページの場所ではなく、CSSの場所を基準に解決されます。 初期設定のCSSを別の場所にコピーする場合は、このルールを考慮してください。 初期設定のアセットをコピーするか、カスタムCSS内のパスを更新します。
