---
description: eCatalogビューアのすべての視覚的なカスタマイズと、ほとんどの動作のカスタマイズは、カスタムCSSを作成することで行います。
keywords: responsive
seo-description: eCatalogビューアのすべての視覚的なカスタマイズと、ほとんどの動作のカスタマイズは、カスタムCSSを作成することで行います。
seo-title: eCatalogビューアのカスタマイズ
...
# eCatalogビューアのカスタマイズ{#customizing-ecatalog-viewer}

eCatalogビューアのすべての視覚的なカスタマイズと、ほとんどの動作のカスタマイズは、カスタムCSSを作成することで行います。
...
カスタムCSSを作成する場合、ビューアはコンテナのDOM要素に`.s7ecatalogviewer`クラスを割り当てることに注意してください。 `style=`コマンドによって渡された外部CSSファイルを使用する場合は、CSSルールの子孫セレクターで`.s7ecatalogviewer`クラスを親クラスとして使用します。 Webページ上で埋め込みスタイルを使用する場合、このセレクターを次のようにコンテナのDOM要素のIDで修飾します。
...
eCatalogビューアに適用されるユーザインターフェイス要素リファレンスドキュメントを次に示します。
