...
* 作成者がバックエンドのScene7 Publishing System内で定義するラベル。
* ビューアによって自動生成されるラベル。
...
SPSで定義されたラベルは、個々のページ画像のUserDataレコードに保存されます。 シンボルベースのラベルと同じです。 eCatalog見開きの両方のページにラベルが定義されている場合、ランドスケープモードでは`MediaSet.LABEL_DELIM`記号を使用してラベルが連結されます。 SPSラベルは、自動的に生成されるラベルよりも優先されますが、シンボルベースのラベルによって上書きされます。

自動生成されるラベルは、eCatalog内のすべてのページに割り当てられる連番です。 シンボルベースのラベルが定義されているか、SPSラベルが定義されている場合、指定された見開きに対して自動的に生成されたラベルは無視されます。
