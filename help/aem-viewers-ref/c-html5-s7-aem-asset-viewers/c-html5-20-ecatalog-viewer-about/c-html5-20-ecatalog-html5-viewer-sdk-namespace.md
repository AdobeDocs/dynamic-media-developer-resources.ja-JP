...
例えば、イベントをリッスンする予定で、 `StatusEvent.NOTF_VIEW_READY` ビューアがScene7 Publishing Systemから供給される場合、完全修飾イベントタイプはで `s7classic.s7sdk.event.StatusEvent.NOTF_VIEW_READY`、イベントリスナーコードは次のようになります。
