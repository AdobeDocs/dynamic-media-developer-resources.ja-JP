...
カスタムCSSを作成する場合、ビューアはコンテナのDOM要素にク `.s7interactiveimage` ラスを割り当てることに注意してください。 コマンドで渡された外部CSSファイルを使用する場合は、CSS `style=` ルールの子孫セレ `.s7interactiveimage` クターでクラスを親クラスとして使用します。 Webページに埋め込みスタイルを追加する場合は、このセレクターを次のようにコンテナのDOM要素のIDで修飾します。
...
以下は、「スプライト」ベースの同じズームインボタンのスタイル設定例です。
