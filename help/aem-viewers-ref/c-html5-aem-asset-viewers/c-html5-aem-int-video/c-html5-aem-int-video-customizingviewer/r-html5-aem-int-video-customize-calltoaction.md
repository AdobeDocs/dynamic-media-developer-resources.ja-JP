...
パネルは、ビデオタイトルを示すヘッダー領域、右上隅の再生ボタン、スクロール可能なグリッドとして表示される実際のインタラクティブスウォッチで構成されます。 パネルを無効にするには、callToActionRecap設定属 [性を使用し](../../../c-html5-aem-asset-viewers/c-html5-aem-int-video/r-html5-aem-int-video-config-attrib/r-html5-aem-int-video-config-attrib-calltoactionrecap.md#reference-3720b68800684ddabf523e9d81644ce6) ます。

誘い文句（CTA：コールトゥアクション）パネルは、常に利用可能なビューア領域全体を占めます。
...
## 誘い文句（CTA：コールトゥアクション）パネルの背景色のCSSプロパティ {#css-properties-of-the-background-color-of-the-call-to-action-panel}
...
ダークグレーの背景で誘い文句（CTA：コールトゥアクション）パネルを設定するには：
...
以下に示すCSSクラスセレクターで、誘い文句（CTA：コールトゥアクション）パネルのヘッダーの外観を制御します。

```
.s7interactivevideoviewer .s7calltoaction .s7header
```

## 誘い文句（CTA：コールトゥアクション）パネルのヘッダーのCSSプロパティ {#css-properties-of-the-call-to-action-panel-header}
...
高さが70ピクセルで、背景色が濃いグレーで、下端にわずかに明るいグレーの2ピクセルの境界線を持つヘッダーを設定するには、次のように記述します。
...
<table id="table_A5E36A5C4C664346B6DAE9A02B36C3D2"> 
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p> 「再生」ボタンのスペースを確保するための右パディング。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
行の高さが70ピクセル、フォントサイズが25ピクセル、白色、左揃えのビデオタイトルを設定するには：
...
次のCSSクラスセレクターで、誘い文句（CTA：コールトゥアクション）パネルの閉じるボタンの外観を制御します。
...
28 x 28ピクセルの再生ボタンを設定するには、次のように記述します。ヘッダーの上端と右端から20ピクセルの位置に配置します。ボタンの4つの状態ごとに異なる画像を表示します。コンポーネントのスプライト画像からアートワークを取得します。
...
以下に示すCSSクラスセレクターで、誘い文句（CTA：コールトゥアクション）パネルのサムネールグリッドビューの外観を制御します。

```
.s7interactivevideoviewer .s7calltoaction .s7thumbnailgridview
```

## 誘い文句（CTA：コールトゥアクション）パネルのサムネールグリッドビューのCSSプロパティ：  {#css-properties-of-the-thumbnail-grid-view-in-the-call-to-action-panel}
...
以下に示すCSSクラスセレクターで、誘い文句(CSS)パネルのサムのセルの外観を制御します。

```
.s7interactivevideoviewer .s7calltoaction .s7thumbcell
```

## 誘い文句（CTA：コールトゥアクション）パネルのサムセルのCSSプロパティ： {#css-properties-of-the-thumbcell-in-the-call-to-action-panel}

<table id="table_9CEBEF6FC7024F02840A581AEEF612B4"> 
 <tbody> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> margin </span> </p> </td> 
   <td colname="col2"> <p> 各サムネールの周囲の水平方向および垂直方向のマージンのサイズ。 </p> <p>実際のサムネールの水平方向の間隔は、.s7thumbcellに設定された左右のマージンの合計 <span class="codeph"> に等しくなりま </span>す。 同じルールが適用されますが、垂直方向の間隔にも適用されます。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
以下に示すCSSクラスセレクターで、誘い文句（CTA：コールトゥアクション）パネルのサムネールの外観を制御します。
...
>[!NOTE]
>
>サムネールでは、属性セ `state` レクターがサポートされます。このセレクターは、サムネールの状態ごとに異なるスキンを適用するのに使用できます。 特に、は現在選 `state="selected"` 択されている画像のサムネールに対応します。は、残 `state="default"` りのサムネールに対応します。は、マウ `state="over"` スのカーソルを合わせたときに使用されます。
...
以下に示すCSSクラスセレクターで、誘い文句（CTA：コールトゥアクション）パネルのサムネールラベルの外観を制御します。
...
<table id="table_E2C9F21EBD9140FD9D20A4BBAD117E2F"> 
 <tbody> 
  ...
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> text-align </span> </p> </td> 
   <td colname="col2"> <p>ラベルの水平方向の配置。 </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <span class="codeph"> font-family </span> </p> </td> 
...
白色を使用するラベルを設定するには、中央揃えで15ピクセルにし、Arialフォントを使用します。
...
サムネールの数が縦方向に収まらない場合は、サムネールの右側に縦方向のスクロールバーが表示されます。 デフォルトでは、誘い文句（CTA：コールトゥアクション）パネルは、サムやスクロールボタンのない小さな縦棒をレンダリングします。 ただし、ビューアのCSSを変更してバーをカスタマイズすることは可能です。

以下に示すCSSクラスセレクターで、誘い文句(CTA)パネルのスクロールバー領域の外観を制御します。
...
幅が22ピクセルで、サムネール領域の上、右または下からのマージンがないスクロールバーを設定するには、次の手順に従います。
...
スクロールバートラックは、スクロールバーの上下のボタンの間の領域です。 トラックの位置と高さが自動的に設定されます。

以下に示すCSSクラスセレクターで、誘い文句（CTA：コールトゥアクション）パネルのスクロールバートラックの外観を制御します。
...
スクロールバーサムは、スクロールトラック領域内で垂直方向に移動します。 その垂直方向の位置は、コンポーネントのロジックによって完全に制御されます。ただし、サムの高さはコンテンツの量に応じて動的に変化するわけではありません。
...
>[!NOTE]
>
>サムでは、属性セレ `state` クターがサポートされます。このセレクターは、次のサムの状態ごとに異なるスキンを適用するのに使用できます。 `"up"`、 `"down"`、、 `"over"`および `"disabled"`。

## 例 {#example-10}

6 x 167ピクセルで、角を3ピクセル丸め、グレーのスクロールバーサムを設定するには、次のように記述します。
...
CSSのtop、left、bottomまたはrightプロパティを使用してスクロールボタンを配置することはできません。ビューアのロジックによって、自動的に配置されます。 インタラクティブビデオビューアのアクション呼び出しパネルでは、スクロールバーのこれらのボタンは使用されないので、初期設定のCSSではサイズが0ピクセルに設定されます。
...
>[!NOTE]
>
>これらのボタンは、属性セ `state` レクターをサポートします。このセレクターは、次のサムの状態ごとに異なるスキンを適用するのに使用できます。 `"up"`、 `"down"`、、 `"over"`および `"disabled"`。

ボタンのツールヒントをローカライズできます。 詳しくは、ユー [ザインターフェイス要素のローカリゼーションを参照してくださ](../../../c-html5-aem-asset-viewers/c-html5-aem-int-video/c-html5-aem-int-video-viewer-localization.md#concept-cbfc39344c494eb7b9f6a272cff0cc74)い。

## 例 {#example-11}

サイズを0に設定し、非表示にして、スクロールボタンを無効にします。
