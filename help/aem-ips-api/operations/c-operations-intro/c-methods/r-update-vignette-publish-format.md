...
| ` *`targetWidth`*` | `xsd:int` | はい | 生成されるビネット表示のターゲット幅をピクセル単位で指定します。 出力ビネットがプライマリビネットと同じサイズになるように、ゼロを使用します。 |
| ` *`targetHeight`*` | `xsd:int` | はい | 生成されるビネット表示のターゲットの高さをピクセル単位で指定します。 出力ビネットがプライマリビネットと同じサイズになるように、ゼロを使用します。 |
