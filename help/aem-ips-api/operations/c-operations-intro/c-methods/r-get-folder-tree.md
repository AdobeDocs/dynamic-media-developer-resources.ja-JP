---
description: 階層ツリー構造でフォルダとサブフォルダを返します。 getFolderTreeの応答は、最大100,000個のフォルダーに制限されます
seo-description: 階層ツリー構造でフォルダとサブフォルダを返します。 getFolderTreeの応答は、最大100,000個のフォルダーに制限されます
...
階層ツリー構造でフォルダとサブフォルダを返します。 getFolderTreeの応答は、最大100,000個のフォルダーに制限されます

構文

## 認証されたユーザータイプ {#section-66ef19149f4d4123a3a99004b5a2743e}
...
| ` *`accessUserHandle`*` | `xsd:string` | いいえ | 管理者が特定のユーザーを装う目的でのみ使用されます。 |
| ` *`accessGroupHandle`*` | `xsd:string` | いいえ | 特定のグループ（会社が属するグループを含む）でフィルターするために使用します。 |
...
このコード例では、会社のハンドルと深さパラメーターを使用して、応答が返す深さのレベルを決定します。 応答には、関連するフォルダーとサブフォルダー配列が含まれます。 深さの値を小さい数値に設定すると、フォルダツリーの深い部分を検索できます。
